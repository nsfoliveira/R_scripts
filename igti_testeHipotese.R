...
qchisq(p=0.95,df = 1) #O valor p unilateral fica
...
dados_anova <- data.frame(Gastos = c(174.770021661909, 161.329206619394, 153.679900850863,	163.790338797433,	141.363480335882,
                                     175.351592994046,
...
ggplot(data = dados_anova, aes(x = Gastos, fill = Estado_Civil)) + geom_density(alpha=0.4)+
  xlim(-50,300)
...
#Com o comando aov(), o R gera a tabela da ANOVA completa anova <- aov(Gastos~ #Variável resposta
Estado_Civil, #Fator que queremos testar se exerce influencia na variável resposta

data = dados_anova)

#Visualize a tabela da ANOVA. Observe o F calculado e o valor p ( Pr > F) summary(anova)
